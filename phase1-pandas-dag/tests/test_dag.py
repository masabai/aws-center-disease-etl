...
def test_dag_task_order(dagbag):
    ...
    assert dag.get_task("validate").upstream_task_ids == {"extract"}
    assert dag.get_task("transform").upstream_task_ids == {"validate"}
    ...
    expected_fail_upstreams = {"extract", "validate", "transform", "load", "observability_drift_check"}
